...
最終更新: 2026-06-15 / リポジトリ: **Exally-test** / 最新コミット: `f85f218`
...
5. **集計（月次集計）**: 月を選ぶと その月の **会社別 請求/入金/残** を表で一覧表示（請求額の大きい順・状態pill付き）＋上部に請求/入金/残の合計カード＋合計行（社数・件数）。**アプリ内で見れる**読み取り専用レポート。「📊この集計をExcel書き出し」で表示中の月を帳簿用.xlsx化（列: 月/会社/件数/請求額/入金額/残額/状態/入金日/メモ＋合計行・金額は数値）。組み立ては `summarize(month,statusF)` に共通化（画面とExcelで共用）。
6. **会社**: 会社マスタ（追加・名前変更・左スワイプ削除・項目チェック・ドラッグ・幅）。
7. **自社**: 発行者/振込先(自由行・文字揃え左中右)・請求書番号トグル・日付(西暦/令和)・判子(アップロード/自動白抜き/サイズmm)。
