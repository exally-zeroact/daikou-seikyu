...
3. ~~入力補助（よく使う行き先/会社のサジェスト）~~ ✅ 完了（commit `ef3cb27`）。行き先=過去明細のdatalist候補（会社優先・頻度順）。会社=直近会社を自動選択（`daiko_lastco_v1`）。金額サジェストは司さん判断で見送り。
4. **出力したら自動でExcelにも反映**（＝請求書/集計を出すと、明細データもExcel/PC側に自動で同期）← 司さん要望・本番クラウド化と絡む後タスク。
5. **アカウント管理**（ログイン→account_idを本物のアカウントに・代行業者ごとデータ分離）← 後タスク。
6. 本番移行: localStorage→Supabase、ブラウザ印刷→サーバPDF。※Exally本番同期は「まだ先」（司さん・2026-06-15）。
