...
最終更新: 2026-06-15 / リポジトリ: **Exally-test** / 最新コミット: `ebb7958`
...
ナビは頻度で階層化（UX専門家の助言＋司さん指摘で 7→5 に集約）。`入力 / 一覧 / 請求 / 入金 / 設定`。タブ内サブ切替は **セグメント**（`.seg`/`.seg-btn.on`/`.seg-panel.on`、`showBillingSeg()`/`showSettingsSeg()`）。
