...
最終更新: 2026-06-15 / リポジトリ: **Exally-test** / 最新コミット: `04ad6b3`
...
4. **入金**: 請求（=会社×月）ごとに **入金済 / 一部入金 / 未入金** と残額を管理。月/状態フィルタ＋上部サマリ(請求/入金/残額)。カードタップで入金額を記録（全額/未入金クイック・入金日・メモ）。状態は請求額と入金額から自動判定。localStorage `daiko_payments_v1`（キー=`account::month::会社名`）。**「📊集計をExcel書き出し」**＝現フィルタの月次集計を帳簿用.xlsx化（列: 月/会社/件数/請求額/入金額/残額/状態/入金日/メモ＋合計行・金額は数値）。
...
2. ~~**月次集計**（その月 誰にいくら請求/入金 の一覧）~~ ✅ 完了（commit `04ad6b3`）。オンスマホ閲覧=入金タブ、帳簿用出力=「集計をExcel書き出し」。さらに専用レポート画面が要るなら追加余地あり。
